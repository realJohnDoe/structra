...
import type { TreeNode, Edge, SubtreeData } from '../components/RepoExplorer';
...
function edgeCost(byPath: Record<string, TreeNode>, a: string, b: string): number {
  const na = byPath[a];
  const nb = byPath[b];
  if (!na || !nb) return 0;
  ...
  const sumLnK = internalNodes.reduce((s, nd) => s + Math.log(Math.max(2, nd.children.length)), 0);
...
function nodeStructCost(nd: TreeNode): number {
  let nH = 0;
  (function walk(n: TreeNode) {
    if (!n.isLeaf && n.children.length > 0) nH += (1 - W) * Math.log(Math.max(2, n.children.length));
    n.children.forEach(walk);
  })(nd);
  return nH;
}

function subtreeH(nd: TreeNode, byPath: Record<string, TreeNode>, edgeArr: Edge[]): SubtreeData {
...
export const useRepoVisualization = (
  ...
  const render = useCallback((nd: TreeNode) => {
    if (!nd || !tree || !svgRef.current) return;
    ...
    const childHvals = children.map(c => subtreeH(c, tree.byPath, edges));
    ...
    for (const e of edges) {
      ...
      if (fi >= 0 && ti >= 0 && fi !== ti) {
        ...
        const ec = edgeCost(tree.byPath, e.from, e.to);
        const oc = optEdgeCost();
        bundles[k].totalH += ec;
        bundles[k].optH += oc;
        bundles[k].count++;
    ...
    // Max subtree H for color normalization
    const maxSubH = Math.max(...childHvals.map(h => h.total), 0.001);
    const optSubH = children.map((c, i) => childHvals[i].optEH);
    const maxWaste = Math.max(...childHvals.map((h, i) => h.eH - h.optEH), 0.001);
    ...
    children.forEach((ch, i) => {
      ...
      g.addEventListener('click', (ev) => {
        ...
        svg.querySelectorAll('g[data-idx]').forEach(gr => {
          ...
          if (path) {
            path.setAttribute('opacity', parseInt(gr.getAttribute('data-idx') || '0') === i ? '1' : '0.35');
          }
        });

        if (onNodeClick) onNodeClick(ch, i);

        // Double-tap to navigate
        tapCount++;
        if (tapTimer) clearTimeout(tapTimer);
        if (tapCount >= 2 && !ch.isLeaf && onNavigate) {
          onNavigate(ch);
          tapCount = 0;
          return;
        }
    ...
    svg.addEventListener('click', (ev) => {
      if (ev.target === svg || (ev.target as Element).tagName === 'circle') {
        render(nd); // clean re-render resets all highlight
      }
    });
    ...
    const dispName = nd.name.length > 13 ? nd.name.slice(0, 12) + '...' : nd.name;
    ...
    const { total: tH } = subtreeH(nd, tree.byPath, edges);
    ...
    const canGoUp = (nd.parent && nd.id !== effectiveRoot.id);
    
    if (canGoUp && onNavigate) {
      centG.style.cursor = 'pointer';
      centG.addEventListener('click', (ev) => {
        ev.stopPropagation();
        onNavigate(nd.parent!);
      });
  ...
  }, [tree, edges, weight, onNodeClick, onNavigate]);
